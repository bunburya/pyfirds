...
from test.values import get_test_run_dir, TEST_DATA_DIR

RUN_DIR = get_test_run_dir(__name__)
SCHEMA_DIR = os.path.join(TEST_DATA_DIR, 'xml_schemas')
...
fulins_schema = etree.XMLSchema(etree.parse(os.path.join(SCHEMA_DIR, 'auth.017.001.02_ESMAUG_FULINS_1.1.0.xsd')))
dltins_schema = etree.XMLSchema(etree.parse(os.path.join(SCHEMA_DIR, 'auth.036.001.03_ESMAUG_DLTINS_1.2.0.xsd')))
...
def test_01_search():
    for from_time, to_time, q in CHECKSUMS:
        results = firds.search(from_time, to_time, q)
        checks = CHECKSUMS[(from_time, to_time, q)]
...
def test_02_download():
    for from_time, to_time, q in CHECKSUMS:
        ...
        for r in results:
            ...
            #if r.file_type == 'FULINS':
            #    logger.debug(f'Validating XML file at {xml_fpath} against schema at {fulins_schema}.')
            #    fulins_schema.assertValid(etree.parse(xml_fpath))
            #elif r.file_type == 'DLTINS':
            #    logger.debug(f'Validating XML file at {xml_fpath} against schema at {dltins_schema}.')
            #    dltins_schema.assertValid(etree.parse(xml_fpath))
            os.remove(xml_fpath)
